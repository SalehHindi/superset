...
class CsvToDatabaseForm(DynamicForm):
    ...
    header = IntegerField(_('Header Row'),
                          description=_('Row containing the '
                                        'headers to use as '
                                        'column names (0 is '
                                        'first line of data). '
    ...
    names = StringField(_('Column Names'),
                        description=_('List of comma-separated '
                                      'column names to use if '
                                      'header row not '
                                      'specified above. '
                                      'Leave empty if header '
                                      'field populated.'),
                        validators=[Optional()],
                        widget=BS3TextFieldWidget(),
                        filters=[lambda x: x or None])
    index_col = IntegerField(_('Index Column'),
                             description=_('Column to use as the '
                                           'row labels of the '
    ...
    squeeze = BetterBooleanField(_('Squeeze'),
                                 description=_('Parse the data as '
                                               'a series (specify '
                                               'this option if the '
                                               'data contains only '
                                               'one column.)'))
    prefix = StringField(_('Prefix'),
                         description=_('Prefix to add to column '
                                       'numbers when no header '
                                       '(e.g. "X" for "X0, X1").'),
                         validators=[Optional()],
                         widget=BS3TextFieldWidget(),
                         filters=[lambda x: x or None])
    mangle_dupe_cols = BetterBooleanField(_('Mangle Duplicate Columns'),
                                          description=_('Specify duplicate '
                                                        'columns as '
                                                        '"X.0, X.1".'))
    skipinitialspace = BetterBooleanField(_('Skip Initial Space'),
                                          description=_('Skip spaces '
                                                        'after delimiter.'))
    skiprows = IntegerField(_('Skip Rows'),
                            description=_('Number of rows to skip '
                                          'at start of file.'),
                            validators=[Optional(),
                                        NumberRange(0, 1E+20)],
                            widget=BS3TextFieldWidget(),
    ...
    escapechar = StringField(_('Escape Character'),
                             description=_('Character used to '
                                           'escape a quoted item.'),
                             validators=[Optional()],
                             widget=BS3TextFieldWidget(),
                             filters=[lambda x: x or None])
    comment = StringField(_('Comment Character'),
                          description=_('Character used to denote the '
                                        'start of a comment.'),
                          validators=[Optional()],
                          widget=BS3TextFieldWidget(),
                          filters=[lambda x: x or None])
