...
class BrowserSession(object):
    ...
    def capture(self, path, wait=15, width=1280, height=800):
        ...
        wait = ui.WebDriverWait(self.driver, 50)
...
class Img(BaseSupersetView):
    ...
    def slice(self, slice_id):
        ...
        # http://localhost:8088/img/slice/331/ gets me in this function
        ...
        with BrowserSession() as browser:
            img = browser.capture(
                slc.slice_url + '&standalone=true', wait=15, width=width, height=height)
    ...
    def dashboard(self, dashboard_id):
        """Assigns a list of found users to the given role."""
        # http://localhost:8088/img/dashboard/1/ gets me in this function
        height = int(request.args.get('height', '800'))
        width = int(request.args.get('width', '800'))
        ...
        with BrowserSession() as browser:
            img = browser.capture(
                dashboard.url, wait=15, width=width, height=height)
            return send_file(img, mimetype='image/png')
        return "Nope"
